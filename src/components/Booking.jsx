...
export default function Booking({ copy }) {
  return (
    <section className="section-shell">
      ...
      <div className="site-container relative grid gap-9 lg:grid-cols-[1fr_0.82fr]">
        <div>
          <p className="section-kicker">{copy.booking.kicker}</p>
          <h2 className="section-title">{copy.booking.title}</h2>
          <p className="mt-5 text-[0.95rem] leading-8 text-bone/72">
            {copy.booking.body}
          </p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {copy.bookingEvents.map((event, index) => (
              <div key={event} className="luxury-card card-reveal flex items-start gap-4 rounded-2xl p-4 transition hover:border-gold/55" style={{ animationDelay: `${index * 80}ms` }}>
        ...
        <aside className="premium-frame relative overflow-hidden rounded-[1.75rem] p-6 text-bone">
          <div className="absolute inset-x-0 top-0 h-px bg-gold/35" />
          <div className="absolute bottom-0 left-8 right-8 h-px bg-teal/20" />
          <div className="relative">
            <Music2 className="text-gold" size={38} />
            <h3 className="mt-5 font-display text-[1.7rem] font-bold">{copy.booking.flowTitle}</h3>
            <ol className="mt-6 space-y-5 text-sm leading-7 text-bone/72">
